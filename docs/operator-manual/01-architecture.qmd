...
![Hospital deployment architecture](../../assets/system-architecture.png){#fig-architecture fig-cap="The review workstation uses the provider-neutral Model API; verified model assets remain on the GPU host."}

| Component | Role |
| --- | --- |
| Review workstation | React/Vite clinician UI, FastAPI review routes, Workspace catalog, local review state, annotations, and export. |
...
| CVAT Online | Optional dense annotation and geometry correction. It is not part of offline acceptance. |
...
The application retains the `/ui` static surface for compatibility with existing local integrations. Normal users and deployments must use `/app/`; `/ui` is legacy compatibility only.
