...
For an **Ultra-widefield** image, the current RETFound and PRISM-DR models are unavailable. Review remains available. If the conservative analysis area was prepared, use **Original** and **Analysis area** to inspect the source image and the same-size masked representation. **Analysis area needs review** or **Analysis area unavailable** means the workstation did not establish a safe automated boundary; keep using the original image for human review. The mask is an engineering aid, not a diagnosis or a validated lesion segmentation.
