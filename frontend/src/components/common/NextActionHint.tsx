...
function nextAction(item: CaseRecord, models: ModelDescriptor[] = []): string {
  ...
  if (!item.clinician_review?.final_grade && item.state !== 'REVIEWED') return 'Confirm DR grade';
  if (item.annotation_confirmation_status !== 'CONFIRMED') return 'Review annotations';
  return 'Complete';
}
