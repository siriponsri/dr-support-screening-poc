...
function CaseRow({ item, onResolve, onReadiness, onQueueAction }: { item: CaseRecord; onResolve: (item: CaseRecord) => void; onReadiness: (item: CaseRecord) => void; onQueueAction: (item: CaseRecord, action: 'EXCLUDE' | 'RESTORE') => void }) {
  return (
    <Tr key={item.image_id} data-testid={`case-row-${item.display_name}`} opacity={item.queue_state === 'EXCLUDED' ? 0.62 : 1} bg={item.queue_state === 'EXCLUDED' ? 'surface.subtle' : undefined} title={item.queue_state === 'EXCLUDED' ? 'Excluded from queue. Restore to return this case to the worklist.' : undefined}>
      <Td><ImageCell item={item} /></Td><Td><PatientEyeCell item={item} onResolve={() => onResolve(item)} /></Td><Td><AIResultCell item={item} /></Td><Td><ReviewStatusCell item={item} /></Td><Td isNumeric whiteSpace="nowrap"><ReviewActionCell item={item} onResolve={() => onResolve(item)} onReadiness={() => onReadiness(item)} onQueueAction={(action) => onQueueAction(item, action)} /></Td>
    </Tr>
  );
}

function PatientGroupRows({ group, onResolve, onReadiness, onQueueAction }: { group: PatientGroup; onResolve: (item: CaseRecord) => void; onReadiness: (item: CaseRecord) => void; onQueueAction: (item: CaseRecord, action: 'EXCLUDE' | 'RESTORE') => void }) {
  return <>
    ...
    {group.eyes.map((eye) => <Fragment key={`${group.key ?? 'unlinked'}-${eye.laterality}`}><Tr><Td colSpan={5} py={2} pl={7} borderBottomWidth="0"><Text fontSize="sm" color="text.secondary" fontWeight="semibold">{eye.label}</Text></Td></Tr>{eye.cases.map((item) => <CaseRow key={item.image_id} item={item} onResolve={onResolve} onReadiness={onReadiness} onQueueAction={onQueueAction} />)}</Fragment>)}
  </>;
}

export function WorklistTable({ cases, groups, onResolve, onReadiness, onQueueAction }: WorklistTableProps) {
  return (
    <TableContainer overflowX={{ base: 'auto', desktop: 'hidden' }} maxW="100%">
      <Table variant="clinical" size="sm" layout="fixed" w="100%" minW={{ base: '720px', desktop: '0' }} sx={{ tableLayout: 'fixed' }}>
        ...
        <Tbody>{groups ? groups.map((group) => <PatientGroupRows key={group.key ?? 'unlinked'} group={group} onResolve={onResolve} onReadiness={onReadiness} onQueueAction={onQueueAction} />) : cases.map((item) => <CaseRow key={item.image_id} item={item} onResolve={onResolve} onReadiness={onReadiness} onQueueAction={onQueueAction} />)}</Tbody>
      </Table>
    </TableContainer>
  );
}
